...
public class MazeManager {
    ...
    //Similar to A* algorithm but adds the max euclidean Distance from the fire as a hueristic as well. 
    public int[][] strategy3(int[][] maze, double q){
        ...
        fringe.add(start);
        ArrayList<Point> closedPoints = new ArrayList<Point>();
        ArrayList<Point> firePoints = new ArrayList<Point>();
        double distance;
        int value;
        while(!fringe.isEmpty()){
            Point curr = fringe.remove();
            mazeCopy = advanceFireOneStep(mazeCopy,mazeCopy.length,q);
            firePoints = getFireSteps(mazeCopy,q);    
            distance = getMaxDistance(firePoints,curr);
            value = (int)distance;
            ArrayList<Point> possibleSteps = generateStepsWithHeuristicinFire(mazeCopy,curr,curr.stepsTaken,value);
            if(curr.equals(goal)){
                ArrayList<Point> path = tracePath(curr, start);
                return pathResult(path,mazeCopy);
            }
    ...
    //Method returns the maximum distance from the current point to the fire
    public double getMaxDistance(ArrayList<Point> firePoints,Point x) {
    	
    	double[] distance = new double[firePoints.size()];
    	
    	for(int i=0;i<firePoints.size();i++) {
    		distance[i] = getEuclideanDistance(x,firePoints.get(i));
    	}
    ...
    //calculates distance
    public double getEuclideanDistance(Point x,Point y) {
    	double xDist = Math.pow(y.x-x.x, 2);
    	double yDist = Math.pow(y.y-x.y, 2);
    	double dist = Math.sqrt(xDist+yDist);
    	
    	return dist;
    }
    // returns an arraylist of where the fire currently is.
    public ArrayList<Point> getFireSteps(int[][] maze,double q){
    	Point p = new Point(null,0,0);
    ...
    private ArrayList<Point> generateStepsWithHeuristicinFire(int[][] maze, Point point, int stepsTaken,int distance){
        ArrayList<Point> steps = new ArrayList<>();
        //Check to see if moving 'up' is possible
        if((point.x - 1 >= 0) && (maze[point.x - 1][point.y] != 1) && (maze[point.x - 1][point.y] != 2))
            steps.add(new Point(point, point.x - 1, point.y, stepsTaken + 1,getHeuristic("up", maze, point.x - 1, point.y, stepsTaken + 1)+distance));
        //Check to see if moving 'down' is possible
        if((point.x + 1 < maze.length) && (maze[point.x + 1][point.y] != 1) && (maze[point.x + 1][point.y] != 2))
            steps.add(new Point(point, point.x + 1, point.y, stepsTaken + 1,getHeuristic("down", maze,point.x + 1, point.y, stepsTaken + 1)+distance));
        //Check to see if moving 'left' is possible
        if((point.y - 1 >= 0) && (maze[point.x][point.y - 1] != 1) && (maze[point.x][point.y - 1] != 2))
            steps.add(new Point(point, point.x, point.y - 1, stepsTaken + 1,getHeuristic("left", maze, point.x, point.y - 1, stepsTaken + 1)+distance));
        //Check to see if moving 'right' is possible
        if((point.y + 1 < maze.length) && (maze[point.x][point.y + 1] != 1) && (maze[point.x][point.y + 1] != 2))
            steps.add(new Point(point, point.x, point.y + 1, stepsTaken + 1, getHeuristic("right", maze, point.x, point.y + 1, stepsTaken + 1)+distance));
        return steps;
    }
